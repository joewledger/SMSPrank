...
if(op_system == "Windows"):
	...
	if(os.system("schtasks /QUERY /TN Prank") != 0):
		...
		task = 'schtasks /Create /SC minute /mo 20 /TN Prank /TR "%s %s %s %s %s %s %s"' % (py_exe,py_file, args.username,args.password,args.from_address,args.to_address,args.reciever_name)
		os.system(task)
else:
	...
	if(op_system == "Linux" or op_system == "Darwin"):
		...
		py_exe = "C:\Python" + sys.version[0:3].replace(".","") + "\python.exe"
		py_file = os.path.realpath(__file__)
		task = "%s %s %s %s %s %s %s" % (py_exe,py_file, args.username,args.password,args.from_address,args.to_address,args.reciever_name)
		cron_job = tab.new(task)
		cron_job.setall('*/20 * * * * ')
		tab.write()
	else: 
